...
class Convolutional(KernelLayer):
    ...
    def update_weight(self, momentum: float, learning_rate: float) -> None:
        # print('layer convo')
        # print('prev delta weight', np.sum(self._prev_weights_delta))
        # print('delta weight', np.sum(self._weights_delta))
        current_weights_delta = learning_rate * self._weights_delta + momentum * self._prev_weights_delta
        self._weights = self._weights - current_weights_delta
        self._prev_weights_delta = current_weights_delta
        self._weights_delta = np.zeros(self._weights_delta.shape)

        current_bias_delta = learning_rate * self._bias_delta + momentum * self._prev_bias_delta
        self._bias = self._bias - current_bias_delta
        self._prev_bias_delta = current_bias_delta
    ...
    def _generate_weight(self):
        ...
        self._weights = Activation.init_weight(self._activator, total_filter_element, self._n_kernel).reshape(self._n_kernel, row, col, n_channel)
