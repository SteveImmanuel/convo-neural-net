...
class Dense(BaseLayer):
    ...
    def update_weight(self, momentum: float, learning_rate: float) -> None:
        ...
        current_weight_delta = learning_rate * self._weights_delta + momentum * self._prev_weights_delta
        self._weights = self._weights - current_weight_delta
        self._prev_weights_delta = current_weight_delta
    ...
    def _generate_weight(self):
        ...
        self._weights = Activation.init_weight(self._activator, self._input_shape[-1], self.__n_nodes).reshape(self.__n_nodes, self._input_shape[-1])
        self._weights_delta = np.zeros(self._weights.shape)
